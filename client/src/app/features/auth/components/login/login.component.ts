...
import { catchError, finalize, interval, of, Subscription, tap } from 'rxjs';
...
export class LoginComponent implements OnDestroy {
  ...
  verifyOtp() {
    ...
    this.loading = true;
    this.authService.verifyOtp(this.userEmail, this.otpCode).pipe(
      tap((response: any) => {
        this.stopOtpTimer();
  ...
  private startOtpTimer(seconds: number) {
    ...
    this.otpSubscription = interval(1000).pipe(
      tap(() => {
